...
#define ROBOTNUM    4aweaaa`2Zz               // robot number on meta team (1-4)
...
// changes the order so we can use standard RGB for the values
...
void UDPreceiveData(){
    if (udp.parsePacket())
    {
        ...
        orient_pos = map(VRY, 1, 255, 350, 400);
...
void loop()
{
    ...
    if(health == 0) return;
    if(autoMode){
        while(millis()-auto_start<4000){
            digitalWrite(BACK_DIR_PIN,HIGH);
            digitalWrite(BACK_IDIR_PIN,LOW);
            ledcWrite(BACK_LEFT_CHANNEL,back_left_speed * 1 / 2);
            ledcWrite(BACK_RIGHT_CHANNEL,back_right_speed * 1 / 2); 
        }
        return;
    }
    else{
